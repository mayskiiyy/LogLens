...
import type { EventGroup, LogEvent, LogSource, QueryFilter, Severity, User, Workspace } from '../types';

export class DemoLogLensClient implements LogLensClient {
  private user: User = { id: 'demo-user', email: 'demo@loglens.dev', role: 'admin' };
  ...
  private events: LogEvent[] = [
    {
      ...
      normalized_message: 'Application initialized successfully in production environment',
    ...
    },
    {
      ...
      normalized_message: 'Failed to process order <NUM> due to connection loss',
  ...
  async createWorkspace(name: string): Promise<Workspace> {
    return { id: `ws-${Date.now()}`, name, owner_id: 'demo-user', created_at: new Date().toISOString() };
  }
  ...
  async queryEvents(_workspaceId: string, filter: QueryFilter): Promise<LogEvent[]> {
    ...
    if (filter.search_query) {
      ...
      if (q.startsWith('fingerprint:')) {
        ...
        result = result.filter((e) => e.message.toLowerCase().includes(q) || e.raw.toLowerCase().includes(q));
      }
    }
  ...
  async exportEvents(_workspaceId: string, format: string, query?: string): Promise<Blob> {
    const events = await this.queryEvents(_workspaceId, { search_query: query });
    if (format === 'csv') {
